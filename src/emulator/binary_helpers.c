/*
 * Helper functions to manipulate a binary string
 */
...
// Returns a mask to be applied to the result of an operation
// using the given bit-width 'sf'
uint64_t getResultMask(bool sf) {
    return getMask(getLength(sf));
}

// Returns true if the 'bit' of 'value' is set
// using little-endian interpretation
bool isSet(uint64_t value, uint64_t bit) {
    return (value >> bit) & 1;
}

// Returns true if the most significant bit of 'value'
// is set using the bit-width 'sf'
...
// Returns a portion of 'value' starting at bit 'littleEnd'
// of 'length' bits with sign extension
...
// Returns a portion of the current instruction starting at bit 'littleEnd'
// of 'length' bits
